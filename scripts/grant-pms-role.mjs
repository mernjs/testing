#!/usr/bin/env node
// Grant (or revoke) PMS panel access for an account. Run with:
//   npm run pms:grant
//
// PMS users live in the SAME `admin_users` collection as the LMS / HRMS
// panels — this script just sets the `roles` array. If the email doesn't exist
// yet it can create the account with a password. This is the ONLY way PMS
// access is granted; there is no self-registration.
...
async function main() {
  ...
  if (!uri) {
    console.error("Missing MONGODB_URI. Run with: node --env-file=.env scripts/grant-pms-role.mjs");
    process.exit(1);
  }
  ...
  const rolesRaw = await askLine(`Roles (comma-separated: ${PMS_ROLES.join(", ")} — blank to REVOKE all PMS access): `);
  ...
  const invalid = roles.filter((r) => !PMS_ROLES.includes(r));
  if (invalid.length > 0) {
  ...
  try {
    ...
    // Merge PMS roles into the existing roles array without disturbing
    // HRMS-only roles (hr, manager, employee) the account may already carry.
    const HRMS_KEEP = new Set(["hr", "manager", "employee"]);

    if (existing) {
      const kept = (existing.roles ?? []).filter((r) => HRMS_KEEP.has(r) || r === "super_admin");
      const nextRoles = Array.from(new Set([...kept.filter((r) => r !== "super_admin"), ...roles]));
      // If super_admin was there and not being removed via a blank PMS grant, keep it.
      ...
      console.log(
        roles.length > 0
          ...
          : `\nRevoked PMS access for ${email}. Remaining roles: [${Array.from(new Set(nextRoles)).join(", ")}].`
      );
