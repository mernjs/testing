#!/usr/bin/env node
/**
 * Realistic FMS Cross-Panel Data Seeder
 *   npm run fms:seed-realistic
 */
...
async function main() {
  ...
  if (!uri) {
    console.error("Missing MONGODB_URI. Run with: node --env-file=.env scripts/seed-fms-realistic.mjs");
    process.exit(1);
  }
  ...
  try {
    ...
    console.log("Seeding Realistic FMS Cross-Panel Data (PRMS, PMS, HRMS, TMS)...");

    // 1. Company Current Accounts
    ...
    for (const acc of companyAccounts) {
      await db.collection("fms_bank_accounts").updateOne({ _id: acc._id }, { $set: acc }, { upsert: true });
    }
    ...
    for (const b of beneficiaries) {
      await db.collection("fms_beneficiaries").updateOne({ _id: b._id }, { $set: b }, { upsert: true });
    }
    ...
    for (const t of transactions) {
      await db.collection("fms_transactions").updateOne({ _id: t._id }, { $set: t }, { upsert: true });
    }
    ...
    for (const pl of paymentLinks) {
      await db.collection("fms_payment_links").updateOne({ _id: pl._id }, { $set: pl }, { upsert: true });
    }
    ...
    for (const inv of invoices) {
      await db.collection("fms_invoices").updateOne({ _id: inv._id }, { $set: inv }, { upsert: true });
    }

    console.log("Successfully seeded realistic FMS financial data across PRMS, PMS, HRMS, and TMS panels!");
  } finally {
    await client.close();
  }
}
