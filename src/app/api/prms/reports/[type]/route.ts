...
import { buildReport, isValidReportType } from "@/lib/prms/reports";
import { buildWorkbook } from "@/lib/prms/xlsx";
import { renderReportPdf } from "@/components/prms/ReportPdf";
...
export async function GET(req: NextRequest, { params }: Context) {
  ...
  const base = `prms-${type}-report-${new Date().toISOString().slice(0, 10)}`;

  if (format === "csv") {
    const csv = toCsv(report.rows, report.columns.map((c) => ({ header: c.header, value: (r: Record<string, unknown>) => r[c.key] })));
