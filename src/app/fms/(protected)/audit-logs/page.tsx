...
import { canViewAuditLog } from "@/lib/fms-roles";
import { listAudit } from "@/lib/fms/audit";
import { formatDateTime } from "@/lib/utils";

export default async function AuditLogsPage({ searchParams }: { searchParams: Promise<{ page?: string }> }) {
  const user = await getCurrentFmsUser();
  if (!user || !canViewAuditLog(user)) redirect("/fms");
  ...
  return (
    <div className="space-y-4">
      <Breadcrumbs items={[{ label: "FMS", href: "/fms" }, { label: "Audit Logs" }]} />
      <div>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between text-sm text-muted-foreground">
          ...
          <div className="flex gap-2">
            <Link href={`/fms/audit-logs?page=${Math.max(page - 1, 1)}`} className={buttonVariants({ variant: "outline", size: "sm" })} aria-disabled={page <= 1} tabIndex={page <= 1 ? -1 : undefined}>
              <ChevronLeft className="size-3.5" data-icon="inline-start" />
              Previous
            </Link>
            <Link href={`/fms/audit-logs?page=${Math.min(page + 1, totalPages)}`} className={buttonVariants({ variant: "outline", size: "sm" })} aria-disabled={page >= totalPages} tabIndex={page >= totalPages ? -1 : undefined}>
              Next
