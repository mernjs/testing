...
import { hrmsLoginAction, type HrmsLoginState } from "./actions";

const initialState: HrmsLoginState = {};
const EMAIL_RE = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

export default function LoginForm() {
  const [state, formAction, pending] = useActionState(hrmsLoginAction, initialState);
    ...
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="w-full max-w-sm">
      <GlassCard>
        <CardHeader>
          ...
          <CardDescription>Sign in to the HR management system.</CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <p className="mt-4 flex items-center justify-center gap-1.5 text-xs text-muted-foreground">
        ...
        HRMS access is role-gated. Access is logged and rate-limited.
      </p>
    </motion.div>
  );
}
