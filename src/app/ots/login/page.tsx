...
export default async function OtsLoginPage() {
  const user = await getCurrentOtsUser();
  if (user) redirect("/ots");
      ...
      <div className="relative hidden w-1/2 flex-col justify-between overflow-hidden bg-gradient-to-br from-primary/10 via-background to-secondary/20 p-10 lg:flex">
        ...
        <div className="relative z-10 max-w-md">
          <h1 className="text-4xl font-black tracking-tight text-foreground">
            ...
            <span className="bg-gradient-to-r from-primary to-yashorbit-coral bg-clip-text text-transparent">one test engine.</span>
          </h1>
          <p className="mt-4 text-muted-foreground">
            Build tests from a shared question bank, assign them to departments, roles,
            applicants and batches, and track results, certificates and analytics in one place.
          </p>
        </div>
