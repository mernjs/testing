...
import { canViewActivityLog } from "@/lib/pms-roles";
import { listActivity } from "@/lib/pms/activity";
...
export default async function ActivityLogPage({ searchParams }: { searchParams: Promise<{ page?: string }> }) {
  ...
  const { items, total, totalPages } = await listActivity({ page, pageSize: 40 });

  return (
    <div className="space-y-4">
      <Breadcrumbs items={[{ label: "PMS", href: "/pms" }, { label: "Activity Log" }]} />
      <div>
              ...
              {items.map((a) => (
                <TableRow key={a._id}>
                  ...
                  <TableCell className="text-muted-foreground">
                    {a.projectId ? (
                      <Link href={`/pms/projects/${a.projectId}`} className="text-primary hover:underline">
                        {a.summary ?? "view project"}
                      </Link>
                    ) : (
                      a.summary ?? "—"
                    )}
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between text-sm text-muted-foreground">
          ...
          <div className="flex gap-2">
            <Link href={`/pms/activity?page=${Math.max(page - 1, 1)}`} className={buttonVariants({ variant: "outline", size: "sm" })} aria-disabled={page <= 1} tabIndex={page <= 1 ? -1 : undefined}>
              <ChevronLeft className="size-3.5" data-icon="inline-start" />
              Previous
            </Link>
            <Link href={`/pms/activity?page=${Math.min(page + 1, totalPages)}`} className={buttonVariants({ variant: "outline", size: "sm" })} aria-disabled={page >= totalPages} tabIndex={page >= totalPages ? -1 : undefined}>
              Next
