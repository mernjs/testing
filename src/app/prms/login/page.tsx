...
export default async function PrmsLoginPage() {
  const user = await getCurrentPrmsUser();
  if (user) redirect(hasPrmsStaffRole(user.roles) ? "/prms" : "/prms/me");
      ...
      <div className="relative hidden w-1/2 flex-col justify-between overflow-hidden bg-gradient-to-br from-primary/10 via-background to-secondary/20 p-10 lg:flex">
        ...
        <div className="relative z-10 max-w-md">
          <h1 className="text-4xl font-black tracking-tight text-foreground">
            Every rupee spent,{" "}
            <span className="bg-gradient-to-r from-primary to-yashorbit-coral bg-clip-text text-transparent">one command centre.</span>
          </h1>
          <p className="mt-4 text-muted-foreground">
            Requisitions, vendors, purchase orders, expenses, assets, subscriptions and payments —
            requested, approved and tracked from a single secure dashboard.
          </p>
        </div>

        <p className="relative z-10 text-xs text-muted-foreground">Private procurement area — not indexed, not public.</p>
      </div>
