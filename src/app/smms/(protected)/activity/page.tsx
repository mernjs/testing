...
import { listAudit, AUDIT_ACTION_LABEL, AUDIT_ENTITIES } from "@/lib/smms/audit";
import { formatDateTime } from "@/lib/utils";

export default async function SmmsActivityPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  const viewer = await getViewer();
  if (!viewer) redirect("/smms/login");
  if (!can(viewer, "VIEW_AUDIT_LOG")) redirect("/smms");
  ...
  const pageHref = (p: number) => {
    ...
    return `/smms/activity?${qs.toString()}`;
  };

  return (
    <div className="space-y-4">
      <PageHeader title="Activity Logs" crumbs={[{ label: "Activity Logs" }]} description={`${total} recorded action${total === 1 ? "" : "s"}: campaigns, ads, posts and media created, AI generations, scheduling, approvals, publishing, integrations and settings. Append-only — never contains generated content or tokens.`} />
      <AuditFilters
        values={filters}
            ...
            <TableBody>
              ...
              {items.map((a) => (
                <TableRow key={a._id}>
                  ...
                  <TableCell><Badge className={a.action === "archive" || a.action === "publish_failed" || a.action === "disconnect" ? "bg-amber-500/15 text-amber-700 dark:text-amber-400" : a.action === "delete" ? "bg-rose-500/15 text-rose-600 dark:text-rose-400" : "bg-primary/10 text-primary"}>{AUDIT_ACTION_LABEL[a.action] ?? a.action}</Badge></TableCell>
                  <TableCell>
