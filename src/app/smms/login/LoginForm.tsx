...
import { smmsLoginAction, type SmmsLoginState } from "./actions";

const initialState: SmmsLoginState = {};
const EMAIL_RE = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

export default function LoginForm() {
  const [state, formAction, pending] = useActionState(smmsLoginAction, initialState);
    ...
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="w-full max-w-sm">
      <GlassCard>
        <CardHeader>
          ...
          <CardDescription>Sign in to the SMMS management panel.</CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <p className="mt-4 flex items-center justify-center gap-1.5 text-xs text-muted-foreground">
        ...
        SMMS access is granted per role by the Super Admin. Access is logged and rate-limited.
      </p>
    </motion.div>
  );
}
