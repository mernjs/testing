...
import { sopLoginAction, type SopLoginState } from "./actions";

const initialState: SopLoginState = {};
const EMAIL_RE = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

export default function LoginForm() {
  const [state, formAction, pending] = useActionState(sopLoginAction, initialState);
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="w-full max-w-sm">
      <GlassCard>
        <CardHeader>
          <div className="mb-1 flex items-center gap-1.5 text-xs font-medium text-primary lg:hidden">
            ...
            {brandify("YashOrbit")} <span className="text-foreground">SOP</span>
          </div>
          <CardTitle className="text-xl">Welcome back</CardTitle>
          <CardDescription>Sign in to the standard operating procedures library.</CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <p className="mt-4 flex items-center justify-center gap-1.5 text-xs text-muted-foreground">
        ...
        SOP access follows your HR and panel roles. Access is logged and rate-limited.
      </p>
    </motion.div>
  );
}
