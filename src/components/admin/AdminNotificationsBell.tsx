...
import { markNotificationReadAction, markAllNotificationsReadAction } from "@/app/admin/(protected)/notifications-actions";
import type { NotificationModule, NotificationPriority } from "@/lib/admin/notifications";
...
export default function AdminNotificationsBell({ items, unread }: { items: BellItem[]; unread: number }) {
  ...
  function openItem(item: BellItem) {
    setOpen(false);
    if (!item.read) startTransition(() => void markNotificationReadAction(item.module, item.id));
    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent align="end" className="max-h-[30rem] w-80 overflow-y-auto p-0">
          ...
          <ul className="pb-1">
            {items.map((item) => (
              <li key={`${item.module}-${item.id}`}>
                ...
                >
                  <span className={cn("mt-1.5 size-1.5 shrink-0 rounded-full", PRIORITY_DOT[item.priority])} />
        ...
        <div className="border-t border-border/60 p-2">
          <Link href="/admin/notifications" onClick={() => setOpen(false)} className="block rounded-md px-2 py-1.5 text-center text-xs font-medium text-muted-foreground hover:bg-muted hover:text-foreground">
