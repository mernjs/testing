...
import { markNotificationsReadAction } from "@/app/aibots/(protected)/actions";
...
export default function AibotsNotificationsBell({ items, unread }: { items: AibotsBellItem[]; unread: number }) {
  const router = useRouter();
  const [open, setOpen] = useState(false);
  const [, startTransition] = useTransition();

  function openItem(item: AibotsBellItem) {
    setOpen(false);
    if (!item.read) startTransition(() => void markNotificationsReadAction([item._id]));
    if (item.link && item.link.startsWith("/")) router.push(item.link);
    else router.refresh();
  }
    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent align="end" className="max-h-[30rem] w-80 overflow-y-auto p-0">
        <PopoverHeader className="flex-row items-center justify-between p-3">
          <PopoverTitle>AIBOTS notifications</PopoverTitle>
          {unread > 0 && (
