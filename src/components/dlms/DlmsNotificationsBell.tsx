...
import { markNotificationsReadAction } from "@/app/dlms/(protected)/actions";
...
export default function DlmsNotificationsBell({ items, unread }: { items: DlmsBellItem[]; unread: number }) {
  const router = useRouter();
  const [open, setOpen] = useState(false);
  const [, startTransition] = useTransition();

  function openItem(item: DlmsBellItem) {
    setOpen(false);
    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent align="end" className="max-h-[30rem] w-80 overflow-y-auto p-0">
        <PopoverHeader className="flex-row items-center justify-between p-3">
          <PopoverTitle>DLMS notifications</PopoverTitle>
          {unread > 0 && (
