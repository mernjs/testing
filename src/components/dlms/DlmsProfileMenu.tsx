...
import { DLMS_ROLE_META, normalizeDlmsRoles, primaryDlmsRoleLabel } from "@/lib/dlms-roles";
    ...
    <div className="shrink-0 border-t border-border/60 p-2">
      <DropdownMenu>
        ...
        <DropdownMenuContent align="start" side="top" className="w-64">
          ...
          <DropdownMenuSeparator />
          <DropdownMenuItem variant="destructive" disabled={isPending} onClick={() => startTransition(() => dlmsLogoutAction())}>
      ...
      <Sheet open={profileOpen} onOpenChange={setProfileOpen}>
        <SheetContent>
          <SheetHeader className="border-b border-border/60">
            ...
            <SheetDescription>Account details for this DLMS session.</SheetDescription>
          </SheetHeader>
          <div className="flex-1 space-y-6 overflow-y-auto p-4">
            ...
            <div className="space-y-2.5 text-sm">
              <div className="flex items-start gap-2">
                ...
                <span>{(roles.includes("super_admin") ? ["super_admin" as const] : normalizeDlmsRoles(roles)).map((r) => DLMS_ROLE_META[r].label).join(", ")}</span>
              </div>
