...
import { saveCashAccountAction } from "@/app/fms/(protected)/cash-accounts/actions";
import type { SerializedCashAccount } from "@/lib/fms/cash-accounts";
...
export default function CashAccountForm({
  ...
  function submit() {
    ...
    startTransition(async () => {
      const res = await saveCashAccountAction(form, account?._id);
      if (!res.ok) {
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      ...
      <SheetContent className="sm:max-w-md">
        <SheetHeader className="border-b border-border/60">
          <SheetTitle>{account ? "Edit Cash Account" : "New Cash Account"}</SheetTitle>
          <SheetDescription>Petty cash or cash-in-hand — no sensitive numbers to protect.</SheetDescription>
        </SheetHeader>
        <div className="flex-1 space-y-4 overflow-y-auto p-4">
          <div className="space-y-1.5">
            <Label>Account Name *</Label>
            <Input value={form.accountName} onChange={(e) => set("accountName", e.target.value)} placeholder="Head Office Petty Cash" />
