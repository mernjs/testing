...
import { markNotificationsReadAction } from "@/app/ots/(protected)/actions";
...
export default function OtsNotificationsBell({ items, unread }: { items: OtsBellItem[]; unread: number }) {
  const router = useRouter();
  const [open, setOpen] = useState(false);
  const [, startTransition] = useTransition();

  function openItem(item: OtsBellItem) {
    setOpen(false);
    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent align="end" className="max-h-[30rem] w-80 overflow-y-auto p-0">
        <PopoverHeader className="flex-row items-center justify-between p-3">
          <PopoverTitle>OTS notifications</PopoverTitle>
          {unread > 0 && (
