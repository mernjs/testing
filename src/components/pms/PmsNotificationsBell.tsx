...
import { markNotificationsReadAction, markAllNotificationsReadAction } from "@/app/pms/(protected)/notifications-actions";
...
export default function PmsNotificationsBell({ items, unread }: { items: BellItem[]; unread: number }) {
    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent align="end" className="max-h-[30rem] w-80 overflow-y-auto p-0">
        ...
        <div className="border-t border-border/60 p-2">
          <Link href="/pms/notifications" onClick={() => setOpen(false)} className="block rounded-md px-2 py-1.5 text-center text-xs font-medium text-muted-foreground hover:bg-muted hover:text-foreground">
