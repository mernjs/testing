...
import {
  markPrmsNotificationsReadAction,
  markAllPrmsNotificationsReadAction,
} from "@/app/prms/(protected)/notifications-actions";
...
export default function PrmsNotificationsBell({ items, unread }: { items: BellItem[]; unread: number }) {
  ...
  function openItem(item: BellItem) {
    setOpen(false);
    if (!item.read) startTransition(() => void markPrmsNotificationsReadAction([item._id]));
    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent align="end" className="max-h-[30rem] w-80 overflow-y-auto p-0">
        ...
        <div className="border-t border-border/60 p-2">
          <Link href="/prms/notifications" onClick={() => setOpen(false)} className="block rounded-md px-2 py-1.5 text-center text-xs font-medium text-muted-foreground hover:bg-muted hover:text-foreground">
