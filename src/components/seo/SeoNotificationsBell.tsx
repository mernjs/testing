...
import { markNotificationsReadAction } from "@/app/seo/(protected)/actions";
...
export default function SeoNotificationsBell({ items, unread }: { items: SeoBellItem[]; unread: number }) {
  const router = useRouter();
  const [open, setOpen] = useState(false);
  const [, startTransition] = useTransition();

  function openItem(item: SeoBellItem) {
    setOpen(false);
    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent align="end" className="max-h-[30rem] w-80 overflow-y-auto p-0">
        <PopoverHeader className="flex-row items-center justify-between p-3">
          <PopoverTitle>SEO notifications</PopoverTitle>
          {unread > 0 && (
