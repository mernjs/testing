...
export default function SopNotificationsBell({ items, unread }: { items: SopBellItem[]; unread: number }) {
  const router = useRouter();
  const [open, setOpen] = useState(false);
  const [, startTransition] = useTransition();

  function openItem(item: SopBellItem) {
    setOpen(false);
    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent align="end" className="max-h-[30rem] w-80 overflow-y-auto p-0">
        <PopoverHeader className="flex-row items-center justify-between p-3">
          <PopoverTitle>SOP notifications</PopoverTitle>
          {unread > 0 && (
