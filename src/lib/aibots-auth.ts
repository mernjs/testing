...
import { normalizeAibotsRoles, hasAibotsAccess } from "@/lib/aibots-roles";

/**
 * AIBOTS panel authentication. A separate cookie / session store from the LMS,
 * HRMS, PMS, TMS, PRMS and SOP panels so sign-in state is independent, but the
 * *identity* store is shared: users live in `admin_users`, and AIBOTS access is
 * gated on the `roles` array there: any account whose roles carry an
 * explicit `aibots_*` role (AI Bots access is never implied by another panel's role).
 */

export const AIBOTS_SESSION_COOKIE = "aibots_session";
...
export interface CurrentAibotsUser {
  id: string;
  email: string;
  /** Raw `admin_users.roles` — every AIBOTS permission check runs over this (see `aibotsCan`). */
  roles: string[];
  /** Explicit AIBOTS roles, for display only. */
  aibotsRoles: ReturnType<typeof normalizeAibotsRoles>;
...
async function getAibotsSessionsCollection() {
  const db = await getDb();
  const collection = db.collection<AibotsSessionDoc>("aibots_sessions");
...
export async function verifyAibotsCredentials(
  email: string,
  password: string
  ...
  // which accounts exist or which have AIBOTS access.
  ...
  if (!hasAibotsAccess(user.roles)) {
    // Correct credentials but no AIBOTS access — do not count as a failed attempt.
    return { ok: false, error: GENERIC };
  }
...
export async function createAibotsSession(adminId: ObjectId): Promise<string> {
  const token = randomBytes(32).toString("hex");
  const sessions = await getAibotsSessionsCollection();
...
export async function destroyAibotsSessionByToken(token: string): Promise<void> {
  const sessions = await getAibotsSessionsCollection();
  await sessions.deleteOne({ tokenHash: hashToken(token) });
}

export async function getSessionAibotsUser(token: string | undefined | null): Promise<CurrentAibotsUser | null> {
  ...
  if (!hasAibotsAccess(roles)) return null;

  return {
    id: user._id.toString(),
    email: user.email,
    roles,
    aibotsRoles: normalizeAibotsRoles(roles),
...
export async function setAibotsSessionCookie(token: string): Promise<void> {
  const store = await cookies();
  store.set(AIBOTS_SESSION_COOKIE, token, {
    httpOnly: true,
...
export async function clearAibotsSessionCookie(): Promise<void> {
  const store = await cookies();
  store.delete(AIBOTS_SESSION_COOKIE);
}

export async function getCurrentAibotsUser(): Promise<CurrentAibotsUser | null> {
  const store = await cookies();
  const token = store.get(AIBOTS_SESSION_COOKIE)?.value;
  return getSessionAibotsUser(token);
}

/**
 * Server-action / route guard. Throws `Unauthorized` when there is no AIBOTS
 ...
 */
export async function requireAibotsUser(...anyOf: string[]): Promise<CurrentAibotsUser> {
 ...
 */
export async function changeOwnAibotsPassword(
  userId: string,
  current: string,
  next: string
