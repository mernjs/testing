...
/**
 * OTS panel authentication. A separate cookie / session store from every other
 * panel so sign-in state is independent, but the *identity* store is shared:
 * users live in `admin_users`, and OTS access is gated on the `roles` array
 * there — an explicit `ots_*` role, or an HRMS / TMS role that implies one
 * (every HRMS employee can take tests; see `ots-roles.ts`). Applicants and
 * students sign in through the External Portal instead.
 */
...
export interface CurrentOtsUser {
  id: string;
  email: string;
  /** Raw `admin_users.roles` — every OTS permission check runs over this (see `otsCan`). */
  roles: string[];
  /** Explicit + implied OTS roles, for display only. */
  otsRoles: ReturnType<typeof effectiveOtsRoles>;
  /** Per-capability overrides from the Super Admin — see `permission-overrides.ts`. */
  permissionOverrides: Record<string, boolean>;
  employeeId: string | null;
  /** TMS student logins (`training_student`) carry their `training_students._id`. */
  studentId: string | null;
  mustChangePassword: boolean;
  createdAt: Date;
  lastLoginAt: Date | null;
...
async function getOtsSessionsCollection() {
  const db = await getDb();
  const collection = db.collection<OtsSessionDoc>("ots_sessions");
...
export async function verifyOtsCredentials(
  email: string,
  password: string
  ...
  // which accounts exist or which have OTS access.
  ...
  if (!hasOtsAccess(user.roles)) {
    // Correct credentials but no OTS access — do not count as a failed attempt.
    return { ok: false, error: GENERIC };
  }
...
export async function createOtsSession(adminId: ObjectId): Promise<string> {
...
export async function destroyOtsSessionByToken(token: string): Promise<void> {
  const sessions = await getOtsSessionsCollection();
  await sessions.deleteOne({ tokenHash: hashToken(token) });
}

export async function getSessionOtsUser(token: string | undefined | null): Promise<CurrentOtsUser | null> {
  ...
  if (!hasOtsAccess(roles)) return null;

  return {
    id: user._id.toString(),
    email: user.email,
    roles,
    otsRoles: effectiveOtsRoles(roles),
...
export async function setOtsSessionCookie(token: string): Promise<void> {
  const store = await cookies();
  store.set(OTS_SESSION_COOKIE, token, {
    httpOnly: true,
...
export async function clearOtsSessionCookie(): Promise<void> {
  const store = await cookies();
  store.delete(OTS_SESSION_COOKIE);
}
...
/**
 * Server-action / route guard. Throws `Unauthorized` when there is no OTS
 ...
 */
export async function requireOtsUser(...anyOf: string[]): Promise<CurrentOtsUser> {
 ...
 */
export async function changeOwnOtsPassword(
  userId: string,
  current: string,
  next: string
