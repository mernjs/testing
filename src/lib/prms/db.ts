...
/**
 * Shared helpers for the PRMS (Procurement & Expense Management) data layer.
 * Every PRMS collection:
 ...
 *
 * Mirrors `src/lib/tms/db.ts` and `src/lib/pms/db.ts`.
 */
...
export async function prmsCollection<T extends { _id: string }>(name: string) {
  const db = await getDb();
  return db.collection<T>(name);
}

// ---------------------------------------------------------------------------
// Sequential code generator (Vendor / PR / PO / GRN / Expense / Asset codes)
// backed by `prms_counters`.
// ---------------------------------------------------------------------------
...
export async function nextSequence(name: string): Promise<number> {
  const db = await getDb();
  const counters = db.collection<CounterDoc>("prms_counters");
